...
static void test_sequence( int key_len, Byte final_value[] )
    ...
    for( i=1; i<50; i++ )
        ...
        Twofish_encrypt( p, p-16, &xkey );

        /* Check that the decryption is correct. */
        Twofish_decrypt( p-16, tmp, &xkey );
...
static void test_odd_sized_keys()
    ...
    Twofish_encrypt( buf, buf, &xkey );
    Twofish_encrypt( buf, buf+16, &xkey );
 ...
 */
void Twofish_encrypt( Byte p[16], Byte c[16], Twofish_key *xkey )
    {
 ...
 * c            16 bytes in which to store the ciphertext
 */
void Twofish_decrypt( Byte c[16], Byte p[16], Twofish_key * xkey )
    {
    UInt32 A,B,C,D,T0,T1;       /* Working variables */
